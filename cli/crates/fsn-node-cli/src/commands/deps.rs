...
/// Return a list of (slot_name, instance_name) pairs that the given service class
/// is likely to depend on, based on the project's ServiceSlots.
///
/// Heuristic: certain service class prefixes are known consumers of specific slots.
fn collect_slot_deps<'a>(
    class: &str,
    slots: &'a fsn_node_core::config::ServiceSlots,
) -> Vec<(&'static str, &'a str)> {
    let mut deps = Vec::new();

    // Determine which slots this service type typically needs.
    let needs_iam  = needs_iam(class);
    let needs_mail = needs_mail(class);
    let needs_db   = needs_db(class);
    let needs_cache = needs_cache(class);

    if needs_iam {
        if let Some(inst) = slots.iam.as_deref() {
            deps.push(("iam", inst));
        }
    }
    if needs_mail {
        if let Some(inst) = slots.mail.as_deref() {
            deps.push(("mail", inst));
        }
    }
    if needs_db {
        // Extra slot names for database
        for (slot, inst) in slots.extra.iter() {
            if slot.contains("database") || slot.contains("db") {
                deps.push(("database", inst.as_str()));
            }
        }
    }
    if needs_cache {
        for (slot, inst) in slots.extra.iter() {
            if slot.contains("cache") {
                deps.push(("cache", inst.as_str()));
            }
        }
    }
...
fn needs_iam(class: &str) -> bool {
    matches!(
        class.split('/').next().unwrap_or(""),
        "git" | "wiki" | "chat" | "collab" | "tasks" | "tickets" | "maps"
    )
}

fn needs_mail(class: &str) -> bool {
    matches!(
        class.split('/').next().unwrap_or(""),
        "git" | "wiki" | "tasks" | "tickets"
    )
}

fn needs_db(class: &str) -> bool {
    matches!(
        class.split('/').next().unwrap_or(""),
        "git" | "wiki" | "collab" | "tasks" | "tickets" | "monitoring"
    )
}

fn needs_cache(class: &str) -> bool {
    matches!(
        class.split('/').next().unwrap_or(""),
        "git" | "chat" | "collab"
    )
