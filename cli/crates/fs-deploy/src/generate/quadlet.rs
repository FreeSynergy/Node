...
///   (e.g. "fs-myproject"). Injected automatically by the deploy engine.
///
/// # Errors
///
/// Returns an error if the instance has no `[container]` block.
pub fn generate(instance: &ServiceInstance, project_network: Option<&str>) -> Result<String> {
    let c = instance.class.container.as_ref().ok_or_else(|| {
        anyhow::anyhow!(
            "{}: generate_quadlet called on native app (no [container] block)",
            instance.name
        )
    })?;
    ...
    out.push_str("[Unit]\n");
    let _ = writeln!(out, "Description=FreeSynergy.Node – {}", instance.name);
    out.push_str("After=network-online.target\n");
    out.push_str("Wants=network-online.target\n\n");

    out.push_str("[Container]\n");
    let _ = writeln!(out, "ContainerName={}", instance.name);
    let _ = writeln!(out, "Image={}:{}", c.image, c.image_tag);
    ...
    if let Some(net) = project_network {
        let _ = writeln!(out, "Network={net}.network");
    }
    ...
    for port in &c.published_ports {
        let _ = writeln!(out, "PublishPort={port}");
    }
    ...
    for tmpfs in &c.tmpfs {
        let _ = writeln!(out, "Tmpfs={tmpfs}");
    }

    for opt in &c.security_opt {
        let _ = writeln!(out, "SecurityLabelDisable={opt}");
    }

    if let Some(hc) = &c.healthcheck {
        let _ = writeln!(out, "HealthCmd={}", hc.cmd);
        let _ = writeln!(out, "HealthInterval={}", hc.interval);
        let _ = writeln!(out, "HealthTimeout={}", hc.timeout);
        let _ = writeln!(out, "HealthRetries={}", hc.retries);
        let _ = writeln!(out, "HealthStartPeriod={}", hc.start_period);
    }
...
pub fn quadlet_path(instance_name: &str) -> std::path::PathBuf {
    ...
    std::path::PathBuf::from(format!(
        "{home}/.config/containers/systemd/{instance_name}.container",
    ))
}
...
pub fn network_path(network_name: &str) -> std::path::PathBuf {
    let home = std::env::var("HOME").unwrap_or_else(|_| "/root".to_string());
    std::path::PathBuf::from(format!(
        "{home}/.config/containers/systemd/{network_name}.network",
    ))
}
...
mod tests {
    ...
    use fs_node_core::{
        config::service::{
            Constraints, ContainerDef, HealthCheck, ModuleRoles, ModuleUi, ServiceClass,
            ServiceContract, ServiceLifecycle, ServiceLoad, ServiceMeta, ServiceSetup, ServiceType,
        },
    ...
    fn quadlet_injects_project_network() {
        let out = generate(&make_instance("forgejo", false), Some("fs-myproject")).unwrap();
        assert!(out.contains("Network=fs-myproject.network"));
    }
    ...
    fn generate_network_contains_project_label() {
        let out = generate_network("fs-myproject", "myproject");
        assert!(out.contains("[Network]"));
