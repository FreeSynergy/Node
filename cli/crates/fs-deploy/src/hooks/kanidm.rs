...
fn collect_oidc_clients<'a>(
    ctx: &HookContext<'a>,
) -> Vec<&'a fs_node_core::state::desired::ServiceInstance> {
    let mut out = Vec::new();
...
fn collect_recursive<'a>(
    modules: &'a [fs_node_core::state::desired::ServiceInstance],
    out: &mut Vec<&'a fs_node_core::state::desired::ServiceInstance>,
) {
    for m in modules {
        if m.class.load.peer_services.contains("kanidm") {
            out.push(m);
        }
