...
pub async fn run(ctx: &HookContext<'_>) -> Result<()> {
    ...
    if !ctx.is_initialized() {
        ...
        let admin_user = "fs-admin";
        let admin_email = format!("admin@{}", ctx.project.project.domain);
        let admin_pass = ctx
            .vault
            .expose("vault_forgejo_admin_password")
    ...
    if ctx.desired.services.iter().any(|m| {
        m.class_key == "auth/kanidm" || m.sub_services.iter().any(|s| s.class_key == "auth/kanidm")
    }) {
