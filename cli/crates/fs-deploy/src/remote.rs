...
use fs_host::{RemoteHost, RemoteSystemd, SshSession};
use fs_node_core::{
...
pub async fn deploy_all_remote(
    ...
    let session = SshSession::connect(remote)
        .await
    ...
    session
        .exec(&format!("mkdir -p {quadlet_dir}"))
        .await?
        .into_result()
    ...
    session
        .write_file(&net_path, net_content.as_bytes())
        .await
    ...
    for instance in &instances {
        ...
        health::wait_for_ready(instance, opts.health_timeout)
            .await
...
async fn write_remote_quadlet_files(
    ...
    session
        .write_file(&qpath, quadlet.as_bytes())
        .await
    ...
    session
        .write_file(&epath, env_content.as_bytes())
        .await
...
async fn run_remote_plugin_configs(
    ...
    for instance in &desired.services {
        ...
        let Some(store_root) = &opts.store_root else {
            warn!(
                "  Skipping remote generate-config for '{}': no store_root set",
                instance.name
            );
            continue;
        };
