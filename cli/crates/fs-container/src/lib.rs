...
//! 4. **Convert** — [`converter::convert`] maps compose services to [`fs_container::ServiceConfig`]
//! 5. **Install** — [`fs_container::QuadletManager`] writes `.container` unit files + daemon-reload
//!
...
// Re-export low-level types from fs-container-ext so dependents don't need
// a direct dependency on that crate.
pub use fs_container_ext::{
