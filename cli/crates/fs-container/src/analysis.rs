...
impl VarType {
    ...
    pub fn infer_from(upper: &str) -> (VarType, u8) {
        ...
        if has_any(
            upper,
            &["_URL", "_URI", "_ENDPOINT", "_BASEURL", "_BASE_URL"],
        ) {
            return (VarType::Url, 90);
        }
        if has_any(
            upper,
            &["_HOST", "_HOSTNAME", "_ADDR", "_ADDRESS", "_SERVER"],
        ) {
...
impl VarRole {
    ...
    pub fn infer_from(upper: &str) -> (VarRole, u8) {
        ...
        if has_any(
            upper,
            &["S3_", "_S3", "MINIO", "STORAGE_", "_STORAGE", "BUCKET"],
        ) {
            return (
                VarRole::Storage {
                    kind: StorageKind::S3,
