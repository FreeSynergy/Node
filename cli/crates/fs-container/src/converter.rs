// Converter — ComposeFile → Vec<ServiceConfig> (fs-container types).
//
...
// Uses fs-container's existing quadlet generation — no socket, no bollard.

use fs_container_ext::{HealthCheck, PortBinding, ServiceConfig, Volume};
...
impl ComposeConverter {
    ...
    /// Build the instance name: `prefix-name` when prefix differs from name, else just `name`.
