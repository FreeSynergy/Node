...
use std::path::Path;
use anyhow::Result;
use fs_node_core::config::{HostConfig, ServiceRegistry, ProjectConfig, VaultConfig,
...
pub async fn run(root: &Path, project: Option<&Path>) -> Result<()> {
    ...
    if !diff.to_deploy.is_empty() {
        println!("{}", fs_i18n::t_with("sync.to-deploy", &[("n", &diff.to_deploy.len().to_string())]));
        for m in &diff.to_deploy { println!("  + {}", m.name); }
    }
    if !diff.to_update.is_empty() {
        println!("{}", fs_i18n::t_with("sync.to-update", &[("n", &diff.to_update.len().to_string())]));
        for m in &diff.to_update { println!("  ~ {}", m.name); }
    }
    if !diff.to_remove.is_empty() {
        println!("{}", fs_i18n::t_with("sync.to-remove", &[("n", &diff.to_remove.len().to_string())]));
