...
pub fn detect(svc: &ComposeService) -> ServiceTypeHint {
    ...
    let port_numbers: Vec<u16> = svc
        .ports
        .iter()
        .filter_map(|p| p.split(':').next_back()?.parse().ok())
        .collect();

    // ── image-name rules (highest priority) ───────────────────────────────────
    match image_base {
        "nginx" | "caddy" | "traefik" | "zentinel" | "haproxy" => {
            return ServiceTypeHint::new(
                "proxy/zentinel",
                format!("image '{image_base}' is a reverse proxy"),
                90,
            )
        }
        ...
        "stalwart" | "postfix" | "exim" | "mailu" | "mailserver" => {
            return ServiceTypeHint::new(
                "mail/stalwart",
                format!("image '{image_base}' is a mail server"),
                85,
            )
        }
        ...
        "forgejo" | "gitea" | "gogs" | "gitlab-ce" | "gitlab-ee" => {
            return ServiceTypeHint::new(
                "git/forgejo",
                format!("image '{image_base}' is a git server"),
                90,
            )
        }
        ...
        "outline" | "bookstack" | "wiki-js" | "wikijs" | "dokuwiki" => {
            return ServiceTypeHint::new(
                "wiki/outline",
                format!("image '{image_base}' is a wiki"),
                85,
            )
        }
        ...
        "kanidm" | "keycloak" | "authentik" | "authelia" => {
            return ServiceTypeHint::new(
                "iam/kanidm",
                format!("image '{image_base}' is an IAM service"),
                90,
            )
        }
        ...
        "tuwunel" | "synapse" | "element-web" | "matrix-org" | "conduit" => {
            return ServiceTypeHint::new(
                "chat/tuwunel",
                format!("image '{image_base}' is a Matrix chat server"),
                85,
            )
        }
        ...
        "cryptpad" | "etherpad" | "hedgedoc" | "excalidraw" => {
            return ServiceTypeHint::new(
                "collab/cryptpad",
                format!("image '{image_base}' is a collaboration tool"),
                80,
            )
        }
        ...
        "vikunja" | "planka" | "wekan" | "focalboard" => {
            return ServiceTypeHint::new(
                "tasks/vikunja",
                format!("image '{image_base}' is a task manager"),
                80,
            )
        }
        ...
        "pretix" | "eventyay" => {
            return ServiceTypeHint::new(
                "tickets/pretix",
                format!("image '{image_base}' is an event/ticketing system"),
                80,
            )
        }
        ...
        "umap" | "openstreetmap" => {
            return ServiceTypeHint::new(
                "maps/umap",
                format!("image '{image_base}' is a maps service"),
                80,
            )
        }
        ...
        "openobserve" | "grafana" | "prometheus" | "victoria-metrics" => {
            return ServiceTypeHint::new(
                ...
                format!("image '{image_base}' is a monitoring system"),
                80,
            )
        }
        ...
        "postgres" | "postgresql" | "timescaledb" => {
            return ServiceTypeHint::new(
                "database/postgres",
                format!("image '{image_base}' is a PostgreSQL database"),
                90,
            )
        }
        ...
        "dragonfly" | "redis" | "keydb" | "valkey" => {
            return ServiceTypeHint::new(
                "cache/dragonfly",
                format!("image '{image_base}' is a cache/key-value store"),
                85,
            )
        }
    ...
    for port in &port_numbers {
        match port {
            80 | 443 | 8080 | 8443 => {
                return ServiceTypeHint::new(
                    "proxy/zentinel",
                    format!("port {port} suggests a web/proxy service"),
                    50,
                )
            }
            25 | 465 | 587 | 993 | 995 | 143 => {
                return ServiceTypeHint::new(
                    "mail/stalwart",
                    format!("port {port} is a standard mail port"),
                    60,
                )
            }
            3000 | 3030 => {
                return ServiceTypeHint::new(
                    "git/forgejo",
                    "port 3000 is common for Forgejo/Gitea",
                    40,
                )
            }
            5432 => {
            ...
            8448 | 8008 => {
                return ServiceTypeHint::new(
                    "chat/tuwunel",
                    "port 8448/8008 is Matrix federation/client port",
                    65,
                )
            }
            _ => {}
