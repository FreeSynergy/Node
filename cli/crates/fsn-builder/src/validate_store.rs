...
impl Issues {
    ...
    fn is_ok(&self) -> bool { self.errors.is_empty() && self.warnings.is_empty() }
}
...
fn validate_entry(entry: &PackageEntry, store_dir: &Path, issues: &mut Issues) {
    ...
    match &entry.icon {
        ...
        Some(icon) if icon.starts_with("http://") || icon.starts_with("https://") => {
            // External URL — not verified here (would require network access)
        }
        Some(_) => {
    ...
    match pkg_type {
        ...
        "container_app" | "" => {
            if let Some(path) = &entry.path {
                ...
                // Check either manifest.toml in subdir OR the path itself is a .toml
                if !manifest_path.exists() && !legacy_toml.with_extension("toml").exists() {
                    // Try legacy: the module TOML is {category}/{name}.toml
