...
impl ComposeAnalyzer {
    ...
    pub fn run(&self, path: &Path, format: &str) -> Result<()> {
        let yaml = std::fs::read_to_string(path)
            .with_context(|| format!("Cannot read {}", path.display()))?;
        let resource = self.analyze(&yaml, path)?;
        match format {
    ...
    pub fn analyze(&self, yaml: &str, source_path: &Path) -> Result<ContainerResource> {
        ...
        for (svc_name, def) in &compose.services {
            for var_name in Self::extract_env_vars(def) {
                variables.push(self.analyze_variable(&var_name, svc_name, &all_service_names));
            }
        }
        ...
        let meta = ResourceMeta {
            ...
            name: Self::capitalize(primary_name),
            description: format!(
                "{} — containerized application for FreeSynergy.",
                Self::capitalize(primary_name)
            ),
    ...
    fn build_services(&self, compose: &ComposeFile, primary_name: &str) -> Vec<ContainerService> {
        compose.services.iter().map(|(name, def)| {
    ...
    fn detect_primary<'a>(&self, compose: &'a ComposeFile) -> Result<(&'a str, &'a ComposeServiceDef)> {
        ...
        compose.services.iter().next()
            .map(|(n, d)| (n.as_str(), d))
            .ok_or_else(|| anyhow::anyhow!("No services found"))
    }

    fn detect_roles_provided(&self, image: &str) -> Vec<Role> {
        ...
        if img.contains("outline") || img.contains("bookstack") || img.contains("wiki")    { roles.push(Role::new("wiki")); }
        if img.contains("stalwart") || img.contains("postfix") || img.contains("maddy")    { roles.push(Role::new("smtp")); }
        ...
        if img.contains("postgres") || img.contains("mysql") || img.contains("mariadb")    { roles.push(Role::new("database")); }
        if img.contains("redis") || img.contains("dragonfly") || img.contains("valkey")    { roles.push(Role::new("cache")); }
        if img.contains("mistral") || img.contains("ollama") || img.contains("llamacpp")   { roles.push(Role::new("llm")); }
        roles
    }

    fn detect_roles_required(&self, vars: &[ContainerVariable]) -> Vec<RoleDep> {
        let mut seen: std::collections::HashSet<String> = std::collections::HashSet::new();
        vars.iter()
            .filter_map(|v| v.role.as_ref())
            .filter(|r| seen.insert(r.as_str().to_owned()))
            .map(|r| RoleDep { role: r.clone(), optional: false })
            .collect()
    }
    ...
    pub fn analyze_variable(
        ...
        let var_type = if upper.contains("SECRET") || upper.contains("PASSWORD")
    ...
    fn detect_auto_source(upper: &str, all_services: &[String]) -> Option<AutoSource> {
        ...
        if let Some(role) = Self::detect_var_role(upper) {
            ...
            return Some(AutoSource::RoleVariable { role, field: field.to_owned() });
        }
        None
    }
    ...
    fn is_infrastructure(image: &str) -> bool {
        ...
        img.contains("postgres") || img.contains("mysql") || img.contains("mariadb")
            || img.contains("redis") || img.contains("dragonfly") || img.contains("memcached")
            || img.contains("valkey") || img.contains("minio") || img.contains("nginx")
            || img.contains("caddy") || img.contains("traefik")
    }
...
pub fn analyze_compose(yaml: &str, source_path: &Path) -> Result<ContainerResource> {
    ComposeAnalyzer.analyze(yaml, source_path)
}
...
"#;
    ...
    fn analyze_forgejo_compose() {
        let result = ComposeAnalyzer.analyze(SAMPLE_COMPOSE, std::path::Path::new("compose.yml")).unwrap();
    ...
    fn database_url_gets_database_role() {
        let var = ComposeAnalyzer.analyze_variable("DATABASE_URL", "app", &["postgres".to_string()]);
