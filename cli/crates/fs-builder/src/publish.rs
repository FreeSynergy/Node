...
pub fn run(path: &Path, store_remote: &str) -> Result<()> {
    ...
    let signature = sign_resource(&raw)?;
    ...
    // Git commit + push
    git_publish(path, &resource.meta.id, store_remote)?;

    println!("✅ Published {} to {}", resource.meta.id, store_remote);
    Ok(())
}
...
fn sign_resource(content: &str) -> Result<String> {
    ...
    let key_path = home_dir().join(".config/fsn/signing.key");
...
fn git_publish(package_dir: &Path, resource_id: &str, remote: &str) -> Result<()> {
    let store_dir = home_dir().join(".local/share/fsn/store-clone");

    // Clone if not present.
    if !store_dir.join(".git").exists() {
        std::fs::create_dir_all(&store_dir).context("Cannot create store clone dir")?;
        git_run(&store_dir, &["clone", remote, "."])?;
    } else {
        git_run(&store_dir, &["pull", "--rebase"])?;
    }
    ...
    // Commit + push.
    git_run(&store_dir, &["add", "."])?;
    let msg = format!("publish: {resource_id}");
    git_run(&store_dir, &["commit", "-m", &msg])?;
    git_run(&store_dir, &["push"])?;
